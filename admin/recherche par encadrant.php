    ...
    <div id="content">
       ...
       <div id="recherche_etudiant">
            <form action="#">
                ...
                <input type="text" id="myInput" onkeyup="myFunction()" placeholder="nom de l'encadrant...">
            </form>
            ...
            <table id="styled-table" >
                ...
                <tbody>
                                <?php 
                                    $sql="SELECT et.id_etudiant as appo,et.nom as nom,et.prenom as prenom,s.note as note ,en.nom_enseignant as en_nom,en.prenom_enseignant as en_prenom, s.id_stage as stage
                                    FROM etudiant as et,enseignant as en,stage as s 
                                    where (et.id_etudiant=s.id_etudiant) and (s.id_enseignant=en.id_enseignant) and (s.id_enseignant IS NOT NULL) ;";
                                    
                                    $result = mysqli_query($link,$sql);
                                    while($data=mysqli_fetch_assoc($result)){
                                    ?>      
                                    <tr>
                                        <td><?php echo $data['appo']?></td>
                                        <td><?php echo $data['nom'] ?></td>
                                        <td><?php echo $data['prenom'] ?></td>
                                        <td><?php echo $data['note'] ?></td>
                                        <td><?php echo $data['en_nom']." ".$data['en_prenom'] ?></td>
                                        <td><a title="Voir le detail" style="color: #565A97; text-align: center;" onclick="window.location.href='visualiser_stage.php?id_stage=<?php echo $data['stage'] ?>'">
